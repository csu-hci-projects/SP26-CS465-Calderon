...
fist-up/down workspace switching, fist-left/right window moves, launcher combo,
and a deliberate open-palm/fist/open-palm close-window combo.
...
uv run airdesk gesture build-tcn-dataset --features-dir data/features/sprint4-swipes-001 --labels-dir data/labels/sprint4-swipes-001 --out data/models/gestures/tcn-v2-sprint4-swipes-001-manifest.json --feature-preset stream-invariant-v2 --target-mode v2-evidence
uv run airdesk gesture train-tcn-v2 --manifest data/models/gestures/tcn-v2-sprint4-swipes-001-manifest.json --out data/models/gestures/tcn-v2-sprint4-swipes-001.pt --epochs 25 --device auto
uv run airdesk gesture evaluate-tcn-v2 --manifest data/models/gestures/tcn-v2-sprint4-swipes-001-manifest.json --model data/models/gestures/tcn-v2-sprint4-swipes-001.pt --out data/evaluations/sprint4-swipes-001-tcn-v2/summary.json --device auto
...
open-hand relative cursor movement, fast index-pinch taps for left click,
index-pinch drag for select/highlight, middle-pinch drag for scrolling, active-window
...
`12.0`; default smoothing is stricter (`0.16` alpha, `4px` output dead zone,
and `10px` raw jitter gate) so small landmark tremor does not turn into visible
pointer shake. `--fist-fold-threshold 0.09` remains available as one closed-hand
...
Pinch timing is tuned for short tap/release clicks: pinch poses can enter after
one frame, tap max defaults to `0.55s`, click cooldown defaults to `0.16s`, and
a clean index tap can still click if release passes through a non-closed
index/middle pinch ambiguity. Closed-hand/forming-fist ambiguity still cancels
the tap. Index pinch keeps moving the cursor and starts a held left button on
drag motion or a `0.35s` hold, while middle pinch drag emits scroll ticks
continuously instead of waiting for a long hold event. Middle pinch also locks
the cursor in place until release; its scroll anchor stays fixed so returning to
the start zone pauses scrolling. A stationary middle pinch can right-click only
on a clean release, not on contact, and scroll releases never right-click.
...
Use `--feature-preset stream-invariant-v2 --target-mode v2-evidence` for the next targeted V2 collection pass. This preset excludes absolute palm position, hand size, raw image-space palm velocity/displacement/peak velocity, raw hand-count setup leakage, and unscaled finger/pinch distances. It feeds hand-scale-normalized palm motion, palm-centered hand-scale-normalized finger/pinch geometry, direction consistency, timing, tracking-present, confidence, and simple finger-count shape features. The older `stream-invariant` preset remains available for replay compatibility, but it still includes raw image-space motion and `hand_scale`; use it as a legacy regression surface, not the new collection default. Diagnostic fields such as `palm_x`, `palm_y`, `hand_scale`, raw `dx`, and raw velocity should remain visible in logs/dashboard so false activations can be inspected, but they should not be classifier inputs unless a later review explicitly justifies them. The older `--target-mode phase` also trains a `recovery` class, but live testing showed that recovery can dominate the output without helping user-facing gesture recognition.
`airdesk public-data ipn-convert` is the first public-dataset importer. It assumes Caden has downloaded the IPN Hand MP4 videos and annotations into an ignored local path such as `data/public/ipn/`. The converter accepts the official Drive annotation files (`classIdx.txt`, `Annot_TrainList.txt`, `Annot_TestList.txt`) and the GitHub `annotation_ipnGesture/` mirror filenames, runs videos through MediaPipe, writes normal AirDesk replay JSONL, maps only IPN `G05 Throw left` / `G06 Throw right` to AirDesk's existing left/right atomic evidence labels as a lateral-motion proxy, exports `FrameFeatureRow` CSVs, and can write a `stream-invariant-v2` / `v2-evidence` manifest. The newer `--label-mode ipn-all` path trains every non-`D0X` IPN gesture as a named evidence head. IPN does not contain AirDesk commands; it is a public-data prior and diagnostic surface, not a deployable live-control model by itself.
The current best all-IPN checkpoint is `data/models/gestures/tcn-v2-ipn-all-w16-80ep-h64-l4.pt`. It improved held-out all-IPN scoring over the first run, but live testing showed high-confidence false activations for open-hand/ordinary motion, especially `Throw up`, `Open twice`, `Zoom out`, and point-like heads. Treat it as a preview/pretraining prior. Do not wire it to desktop actions.
All-IPN custom heads now pass through a mode-aware learned-recognition filter before the dashboard shows a plain-language "Recognized" callout. The default diagnostic mode is `command`, with only the lateral throw proxies enabled; `cursor` enables click and zoom heads; `zoom-media` isolates zoom heads; `all-ipn` or `--debug-all-heads` keeps the filtered debug view. IPN point heads are suppressed because MediaPipe pose logic is the cleaner path for pointing if AirDesk needs it later. The filter requires per-head threshold, top-vs-runner-up margin, short persistence, and per-hand cooldown. Use `airdesk gesture replay-tcn-v2-log` on live JSONL prediction logs before trusting a mode/filter live.
...
`airdesk gesture train-tcn-v2` trains the optional sequence-evidence TCN over the v2 manifest. The v2 trainer now defaults to a residual, dilated causal TCN with per-frame layer normalization, dropout, weighted/focal BCE for sparse evidence heads, calibrated per-head threshold metadata, receptive-field metadata, and schema-versioned checkpoints; older schema-1 v2 checkpoints still load for replay compatibility. `train-tcn-v2`, `evaluate-tcn-v2`, `diagnose-tcn-v2-events`, and `holdout-tcn-v2` support `--device auto|cpu|cuda`; the v2 path caches source feature CSV rows and moves batches to CUDA instead of repeatedly reloading overlapping windows or forcing the whole manifest onto GPU memory. `airdesk gesture evaluate-tcn-v2` converts evidence heads into the existing replay event decoder, using `start` evidence to help boundary-backed stroke activation and `end` evidence to force release/background instead of preserving boundary heads only as passive metadata. Offline v2 evaluation decodes a deduplicated all-row evidence stream so the window remains compute context rather than the semantic gesture unit. It also supports `--early-match-tolerance-seconds` so causal peaks that fire slightly before a hand-labeled event start can be counted intentionally instead of as both a miss and a false activation. `airdesk gesture holdout-tcn-v2` trains and scores v2 on the filename-ordered source split used by DTW/legacy TCN holdout; on `sprint4-swipes-001`, the first schema-2 source holdout scored only `2/4` held-out swipes with `5` false activations even though same-source replay looked much stronger, so current v2 numbers should be treated as optimistic until a targeted held-out V2 slice exists. `airdesk gesture watch-tcn-v2` is the no-action live/replay preview for schema-2 evidence checkpoints: it runs the shared v2 checkpoint per visible hand stream, defaults to a resizable dashboard preview with webcam landmarks, per-hand evidence bars, decoded-gesture history, emit-vs-peak delay, tracker timing summaries, and the current motion features driving each score (`pos`, hand scale, normalized `dx`, peak x velocity, and direction consistency), decodes candidates with the same start/end-aware event decoder for AirDesk stroke-head checkpoints, avoids flushing half-finished live events before release evidence, and can write predictions/candidates to JSONL with `--events-out`. Custom-head checkpoints such as all-IPN display their strongest evidence heads, show a plain-language recognized-gesture callout when the top custom head crosses `--evidence-threshold`, keep a short recent-recognition history, and disable the AirDesk swipe decoder/candidate output so live preview does not score `ipn_*` heads as `swipe_left` / `swipe_right`. The old compact camera overlay remains available with `--preview-layout camera`, and `--camera-buffer-size` defaults to `1` on this command to reduce live camera backlog where OpenCV honors it. Terminal candidate lines include both the current emit time and the earlier peak time because live decoding intentionally waits for release/recovery evidence, but the dashboard is now the primary live feedback surface. This is still replay/evaluation/live-preview tooling only; it does not route learned swipes into desktop actions.
For custom evidence-head manifests such as all-IPN, use `airdesk gesture evaluate-tcn-v2-heads` for held-out model quality. It scores each evidence head on the final frame of each causal window and writes per-head precision/recall/F1, macro/micro summaries, and a gesture-head confusion table; the older `evaluate-tcn-v2` command remains AirDesk swipe-decoder evaluation. Use `airdesk gesture evaluate-tcn-v2-boundaries` when checking sparse `start` / `end` heads: it deduplicates all-row causal predictions, turns thresholded boundary evidence into peak events, and reports temporal precision/recall/F1 at configurable match tolerances.
