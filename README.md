...
`airdesk gesture chart-record` is the structured "Guitar Hero for swipes" collection path. It takes compact charts such as `RR | rest | RL | rest | RRR`, shows get-ready/stroke/reset/rest prompts in the preview, records the replayable landmark stream, and writes coarse chart-derived stroke/recovery/event labels by default. Use it for fast chained data collection; treat the generated labels as prompt-timing labels that may still need manual refinement before final training.
