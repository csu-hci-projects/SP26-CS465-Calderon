...
Use `--target-mode v2-evidence` for the new TCN v2 surface. It keeps windows as causal compute context, but stores per-frame decoder-facing evidence heads: `intentional_motion`, `stroke_left`, `stroke_right`, `start`, and `end`. This is the target shape for the next continuous data slice; old replay data should be used as regression coverage, not final V2 proof.
