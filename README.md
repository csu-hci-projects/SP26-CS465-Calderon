...
`airdesk public-data ipn-convert` is the first public-dataset importer. It assumes Caden has downloaded the IPN Hand MP4 videos and annotations into an ignored local path such as `data/public/ipn/`. The converter accepts the official Drive annotation files (`classIdx.txt`, `Annot_TrainList.txt`, `Annot_TestList.txt`) and the GitHub `annotation_ipnGesture/` mirror filenames, runs videos through MediaPipe, writes normal AirDesk replay JSONL, maps only IPN `G05 Throw left` / `G06 Throw right` to AirDesk's existing left/right atomic evidence labels as a lateral-motion proxy, exports `FrameFeatureRow` CSVs, and can write a `stream-invariant-v2` / `v2-evidence` manifest. IPN does not contain AirDesk swipe gestures; Jester remains a later clip-level pretraining candidate, and IPN is the first public-data experiment because it is continuous and includes natural non-gesture hand motion.
The all-IPN evidence-head training gate is prepared but not launched. The reviewed script is `scripts/train-ipn-all-tcn-v2.sh`; run it only after Caden explicitly confirms, preferably with `LOG_PATH=data/logs/tcn-v2-ipn-all-80ep-h64-l4-overnight.log nohup bash scripts/train-ipn-all-tcn-v2.sh >/dev/null 2>&1 &`.
