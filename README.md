...
uv run airdesk control run --backend mediapipe --device /dev/video0 --width 640 --height 480 --fps 30 --fourcc MJPG --max-num-hands 2 --events-out data/logs/control-live-dry-run.jsonl --show
...
open-hand relative cursor movement, and JSONL control logs. Pointer
button/scroll real execution is still dry-run only until a real input injection
adapter is installed and tested.
