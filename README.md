...
All-IPN custom heads now pass through a mode-aware learned-recognition filter before the dashboard shows a plain-language "Recognized" callout. The default diagnostic mode is `command`, with only the lateral throw proxies enabled; `cursor` enables point/click/double-click heads; `zoom-media` enables zoom heads; `all-ipn` or `--debug-all-heads` keeps the full evidence view for debugging. The filter requires per-head threshold, top-vs-runner-up margin, short persistence, and per-hand cooldown. Use `airdesk gesture replay-tcn-v2-log` on live JSONL prediction logs before trusting a mode/filter live.
