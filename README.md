...
runtime/live-action wiring, and live preview helpers can evolve without turning
the entrypoint back into one large command file. Dry-run action routing and
guarded Hyprland execution are isolated in `src/airdesk/cli_runtime.py` so the
safety boundary is easy to audit.
