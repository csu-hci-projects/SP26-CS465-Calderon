...
def test_extract_feature_rows_include_motion_and_pose_features(
    ...
    assert rows[1].palm_vx_per_hand_scale == pytest.approx(
        rows[1].palm_vx / rows[1].hand_scale
    )
    assert rows[1].palm_window_peak_abs_vx_per_hand_scale == pytest.approx(
        rows[1].palm_window_peak_abs_vx / rows[1].hand_scale
    )
    assert rows[1].pinch_distance_per_hand_scale == pytest.approx(
        rows[1].pinch_distance / rows[1].hand_scale
    )
