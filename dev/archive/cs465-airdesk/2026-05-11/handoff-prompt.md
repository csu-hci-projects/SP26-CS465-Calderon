...
- Recognition V2 pivot: after the deep research report and live TCN misses, stop treating the current TCN as the recognizer architecture. It is a scaffold. The first safe slice is implemented: `src/airdesk/gestures/motion.py`, `airdesk gesture spot-motion`, and `airdesk gesture evaluate-motion`. Initial replay evidence says this deterministic baseline is a feature/tracking diagnostic, not a live-control recognizer: `sprint4-swipes-001` default mapping matched 0/16 with 12 false activations; flipped mapping matched 5/16 with 12 false activations; stricter flipped dx 1.0 matched 3/16 with 5 false activations; `sprint4-chained-003` default matched 4/10 with 3 repeated fires and 0 false activations. The first TCN v2 surface is also implemented: `build-tcn-dataset --target-mode v2-evidence` writes framewise `intentional_motion`, `stroke_left`, `stroke_right`, `start`, and `end` evidence; `train-tcn-v2` trains the optional sequence-evidence model; `evaluate-tcn-v2` routes stroke evidence through the replay event decoder; `diagnose-tcn-v2-events` writes per-event v2 diagnostics. The first old-data v2 smoke validated plumbing but not quality: `sprint4-swipes-001` matched `0/16` at `0.35` thresholds and `1/16` at permissive `0.30` thresholds, while `sprint4-chained-003` stayed `0/10`. Keep broad combo collection paused and live desktop actions disabled.
- The first pre-training TCN v2 architecture cleanup, replay check, live feel-test, source-level holdout, and feature-contract audit are complete. `train-tcn-v2` now writes schema-2 residual dilated causal TCN checkpoints with per-frame layer normalization/dropout, default 29-frame receptive field, weighted/focal BCE, extra weighting for sparse `start` / `end` positives, calibration threshold metadata, per-head metrics, and batched prediction. Schema-1 v2 checkpoints still load. `evaluate-tcn-v2` now uses `start` and `end` evidence in decoder scoring and supports `--early-match-tolerance-seconds` for causal peaks just before hand-labeled event starts. Same-source old-data replay can score `sprint4-swipes-001` at `16/16` with a 0.25 s early tolerance but still has 5 negative false activations; the new `airdesk gesture holdout-tcn-v2` source split is more honest and scored only `2/4` held-out swipes with `5` false activations. `sprint4-chained-003` same-source replay scores `8/10` with 0 false activations, 3 repeated fires, and high latency. New V2 manifests should use `--feature-preset stream-invariant-v2`, which excludes absolute palm position, raw image-space motion, `hand_scale`, `hand_count`, and unscaled finger/pinch geometry from classifier input while preserving those fields in logs/dashboard diagnostics. `airdesk gesture watch-tcn-v2` now provides a schema-2 no-action live/replay dashboard with webcam landmarks, per-hand evidence bars, recent decoded gestures, emit/peak delay, tracker timing, motion-feature diagnostics, start/end-aware candidate decoding, no open-event flushing before release evidence, and optional JSONL logging through `--events-out`. The old compact overlay is still available with `--preview-layout camera`; the dashboard is now the primary live feedback surface. Next step: targeted held-out V2 data with explicit twist/desk-motion negatives plus near/far and left/center/right invariance checks, then negative-motion intent rejection plus repeated-fire/boundary timing before any live action wiring.
- New public-dataset pivot: the primary-source survey is now in `public-dataset-survey.md`. IPN Hand is the first public-data experiment because it is continuous, RGB-webcam based, CC BY 4.0, includes natural non-gesture hand motion, and has lateral `G05 Throw left` / `G06 Throw right` classes. Jester is larger but remains a later clip-level pretraining candidate. The first importer is `airdesk public-data ipn-convert`; it maps only IPN `G05` / `G06` to AirDesk's existing left/right atomic evidence labels as a lateral-motion proxy, writes normal AirDesk replay/labels/features, and can build a `stream-invariant-v2` / `v2-evidence` manifest. IPN does not contain AirDesk swipe gestures, so keep AirDesk recordings as the pass/fail authority. Do not train every combo as its own TCN class; train atomic gesture/boundary evidence and put combo interpretation in a second command-grammar layer over emitted events.
- IPN local data update: official IPN Hand annotations and all five video archives are downloaded under ignored `data/public/ipn/`; extraction produced 200 `.avi` files under `data/public/ipn/videos/`. The importer now accepts the official Drive annotation filenames directly. A one-video, 120-frame smoke conversion succeeded and wrote `data/public/ipn/airdesk-smoke/tcn-v2-ipn-smoke-manifest.json`.
- IPN-only model update: the first public-data checkpoint,
  `data/models/gestures/tcn-v2-ipn-train-atomic-10ep.pt`, was trained only from
  `data/public/ipn/airdesk-train/tcn-v2-ipn-train-manifest.json`; no AirDesk
  recordings were mixed in. It covered 148 IPN train videos and 296 mapped
  `G05` / `G06` throw-left/right segments. Held-out IPN replay on 52 test videos
  / 104 mapped segments matched `99/104` at permissive thresholds but produced
  `2,183` false activations; stricter `0.80/0.45/0.80` thresholds matched
  `88/104` with `247` false activations; `0.90/0.50/0.90` matched `52/104` with
