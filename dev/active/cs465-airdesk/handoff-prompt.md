...
- Recognition V2 pivot: after the deep research report and live TCN misses, stop treating the current TCN as the recognizer architecture. It is a scaffold. The plan review is complete and the first safe slice is implemented: `src/airdesk/gestures/motion.py`, `airdesk gesture spot-motion`, and `airdesk gesture evaluate-motion`. Initial replay evidence says this deterministic baseline is a feature/tracking diagnostic, not a live-control recognizer: `sprint4-swipes-001` default mapping matched 0/16 with 12 false activations; flipped mapping matched 5/16 with 12 false activations; stricter flipped dx 1.0 matched 3/16 with 5 false activations; `sprint4-chained-003` default matched 4/10 with 3 repeated fires and 0 false activations. Next session should inspect motion-baseline false activations, repeated fires, weak-left misses, hand ids, and direction metadata before any live preview. Do not collect broad combo data or wire live desktop actions.
