...
- Latest two-hand pivot: stop combo data collection until the pipeline supports two visible hands. The old chart combo recordings under `sprint4-gpu-swipes-002-structured` were deleted because collection used the one-hand default and feature export currently consumes only `frame.hands[0]`. Future combo collection should use `--max-num-hands 2`, but only after feature export creates per-hand rows with independent motion history and DTW/TCN/event decoding score and merge events across hands. The `sprint4-gpu-swipes-002-singles` batch remains local but should be treated as legacy/single-hand-only until reviewed or recollected.
