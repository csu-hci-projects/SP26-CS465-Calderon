...
- The first pre-training TCN v2 architecture cleanup and replay check are complete. `train-tcn-v2` now writes schema-2 residual dilated causal TCN checkpoints with per-frame layer normalization/dropout, default 29-frame receptive field, weighted/focal BCE, extra weighting for sparse `start` / `end` positives, calibration threshold metadata, per-head metrics, and batched prediction. Schema-1 v2 checkpoints still load. `evaluate-tcn-v2` now uses `start` and `end` evidence in decoder scoring and supports `--early-match-tolerance-seconds` for causal peaks just before hand-labeled event starts. Current old-data regression: `sprint4-swipes-001` scores `16/16` with a 0.25 s early tolerance but still has 5 false activations, all on normal-desk-motion negatives; `sprint4-chained-003` scores `8/10` with 0 false activations, 3 repeated fires, and high latency. `airdesk gesture watch-tcn-v2` now provides a schema-2 no-action live/replay dashboard with webcam landmarks, per-hand evidence bars, recent decoded gestures, emit/peak delay, tracker timing, start/end-aware candidate decoding, no open-event flushing before release evidence, and optional JSONL logging through `--events-out`. The old compact overlay is still available with `--preview-layout camera`; the dashboard is now the primary live feedback surface. Next step: run a live no-action v2 feel-test, then tighten negative-motion intent rejection plus repeated-fire/boundary timing before targeted V2 recording if the evidence still demands it.
