...
- Recognition V2 pivot: after the deep research report and live TCN misses, stop treating the current TCN as the recognizer architecture. It is a scaffold. The first safe slice is implemented: `src/airdesk/gestures/motion.py`, `airdesk gesture spot-motion`, and `airdesk gesture evaluate-motion`. Initial replay evidence says this deterministic baseline is a feature/tracking diagnostic, not a live-control recognizer: `sprint4-swipes-001` default mapping matched 0/16 with 12 false activations; flipped mapping matched 5/16 with 12 false activations; stricter flipped dx 1.0 matched 3/16 with 5 false activations; `sprint4-chained-003` default matched 4/10 with 3 repeated fires and 0 false activations. The first TCN v2 surface is also implemented: `build-tcn-dataset --target-mode v2-evidence` writes framewise `intentional_motion`, `stroke_left`, `stroke_right`, `start`, and `end` evidence; `train-tcn-v2` trains the optional sequence-evidence model; `evaluate-tcn-v2` routes stroke evidence through the replay event decoder. The first old-data v2 smoke validated plumbing but not quality: `sprint4-swipes-001` matched `0/16` at `0.35` thresholds and `1/16` at permissive `0.30` thresholds, while `sprint4-chained-003` stayed `0/10`. Next session should do a staff-level review/refactor before recording targeted V2 data. Find bugs, dead code, oversized files/functions, weak boundaries, and missing tests; keep broad combo collection paused and live desktop actions disabled.
- CLI cleanup has started without changing `airdesk.cli:app`: offline TCN commands are in `src/airdesk/cli_tcn.py`, label/features commands are in `src/airdesk/cli_labeling.py`, small camera/profile/Hyprland commands are in `src/airdesk/cli_system.py`, and shared helpers are in `src/airdesk/cli_support.py`. Keep future CLI refactors behavior-preserving and test help output.
