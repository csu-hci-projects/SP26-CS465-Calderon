...
- `airdesk gesture chart-record` now supports fast structured collection from compact charts such as `RR | rest | RL | rest | RRR`. It expands the chart into live get-ready/stroke/reset/rest prompts, waits for space before countdown by default, records the replayable landmark stream, and writes coarse chart-derived stroke/recovery/event labels. Treat those labels as weak prompt-timing labels until reviewed.
