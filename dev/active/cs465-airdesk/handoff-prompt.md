...
- CLI cleanup has started without changing `airdesk.cli:app`: offline TCN commands are in `src/airdesk/cli_tcn.py`, label/features commands are in `src/airdesk/cli_labeling.py`, small camera/profile/Hyprland commands are in `src/airdesk/cli_system.py`, shared helpers are in `src/airdesk/cli_support.py`, and live preview/status helpers are in `src/airdesk/cli_live.py`. `src/airdesk/cli.py` is still about 3,371 LOC, so the next session should keep pushing the review/refactor more aggressively before recording targeted V2 data. Good first target: extract recording/collection/chart-prompt ownership into a dedicated module, then isolate runtime/live-action boundaries in a later chunk. Preserve public command behavior, test help/output surfaces, and keep dry-run safety intact.
