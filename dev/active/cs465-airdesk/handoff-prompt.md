...
- The first pre-training TCN v2 architecture cleanup and replay check are complete. `train-tcn-v2` now writes schema-2 residual dilated causal TCN checkpoints with per-frame layer normalization/dropout, default 29-frame receptive field, weighted/focal BCE, extra weighting for sparse `start` / `end` positives, calibration threshold metadata, per-head metrics, and batched prediction. Schema-1 v2 checkpoints still load. `evaluate-tcn-v2` now uses `start` and `end` evidence in decoder scoring and supports `--early-match-tolerance-seconds` for causal peaks just before hand-labeled event starts. Current old-data regression: `sprint4-swipes-001` scores `16/16` with a 0.25 s early tolerance but still has 5 false activations, all on normal-desk-motion negatives; `sprint4-chained-003` scores `8/10` with 0 false activations, 3 repeated fires, and high latency. Next step: tighten negative-motion intent rejection plus repeated-fire/boundary timing before targeted V2 recording.
