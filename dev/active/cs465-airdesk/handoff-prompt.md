...
- The first pre-training TCN v2 architecture cleanup, replay check, live feel-test, and source-level holdout are complete. `train-tcn-v2` now writes schema-2 residual dilated causal TCN checkpoints with per-frame layer normalization/dropout, default 29-frame receptive field, weighted/focal BCE, extra weighting for sparse `start` / `end` positives, calibration threshold metadata, per-head metrics, and batched prediction. Schema-1 v2 checkpoints still load. `evaluate-tcn-v2` now uses `start` and `end` evidence in decoder scoring and supports `--early-match-tolerance-seconds` for causal peaks just before hand-labeled event starts. Same-source old-data replay can score `sprint4-swipes-001` at `16/16` with a 0.25 s early tolerance but still has 5 negative false activations; the new `airdesk gesture holdout-tcn-v2` source split is more honest and scored only `2/4` held-out swipes with `5` false activations. `sprint4-chained-003` same-source replay scores `8/10` with 0 false activations, 3 repeated fires, and high latency. `airdesk gesture watch-tcn-v2` now provides a schema-2 no-action live/replay dashboard with webcam landmarks, per-hand evidence bars, recent decoded gestures, emit/peak delay, tracker timing, motion-feature diagnostics, start/end-aware candidate decoding, no open-event flushing before release evidence, and optional JSONL logging through `--events-out`. The old compact overlay is still available with `--preview-layout camera`; the dashboard is now the primary live feedback surface. Live wrist-twist/lightbulb motion can fire as swipes even though the model omits absolute palm position, because projected normalized dx/velocity/consistency and finger-relative motion still change. Next step: targeted held-out V2 data with explicit twist/desk-motion negatives, then negative-motion intent rejection plus repeated-fire/boundary timing before any live action wiring.
