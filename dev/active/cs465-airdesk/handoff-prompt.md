...
- The first pre-training TCN v2 architecture cleanup is complete. `train-tcn-v2` now writes schema-2 residual dilated causal TCN checkpoints with per-frame layer normalization/dropout, default 29-frame receptive field, weighted/focal BCE, extra weighting for sparse `start` / `end` positives, calibration threshold metadata, per-head metrics, and batched prediction. Schema-1 v2 checkpoints still load. `evaluate-tcn-v2` now uses `start` and `end` evidence in decoder scoring rather than preserving those heads only as metadata. Next step: replay-check this stronger architecture on the old regression data before recording targeted V2 data.
