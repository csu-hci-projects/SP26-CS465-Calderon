...
- First pre-training TCN v2 architecture cleanup and replay check are complete. New v2 checkpoints are schema `2` residual dilated causal TCNs with per-frame layer normalization/dropout, default 29-frame receptive field, weighted/focal BCE with extra sparse-boundary weighting, per-head calibration threshold metadata, per-head metrics, and batched manifest prediction. Schema-1 v2 checkpoints still load for replay compatibility. `evaluate-tcn-v2` lets `start` boost boundary-backed stroke activation and lets `end` force release/background pressure; it now also supports `--early-match-tolerance-seconds` for causal peaks just before hand-labeled event starts. On current old-data regression, isolated swipes score `16/16` with 5 negative false activations when using a 0.25 s early tolerance; `sprint4-chained-003` scores `8/10` with 0 false activations but 3 repeated fires and high latency. `airdesk gesture watch-tcn-v2` now gives schema-2 a no-action live/replay dashboard with webcam landmarks, per-hand evidence bars, decoded-gesture history, emit/peak delay, tracker timing, start/end-aware decoding, no open-event flushing before release evidence, and JSONL logging through `--events-out`. Next: rerun a live no-action v2 feel-test through the dashboard, then tighten negative-motion intent rejection and repeated-fire timing if the evidence still demands it before targeted V2 collection.
...
1. Run `watch-tcn-v2` through the no-action live dashboard and save the JSONL
   output; use the dashboard's evidence bars, decoded-gesture list, emit/peak
   delay, and timing summaries instead of relying on terminal lines alone.
2. Use `diagnose-tcn-v2-events` plus the live preview logs to tighten remaining
   negative-motion false activations and chained repeated fires if they still
   appear outside contaminated-negative explanations.
3. Keep old replay data (`sprint4-swipes-001`, `sprint4-chained-003`, and the
...
4. After that gate, collect a targeted continuous V2 training/test slice:
   repeated same-direction swipes, alternating swipes, weak/tiny lefts, natural
   desk-motion negatives, hand enters/leaves frame, near/far starts, and two
   visible hands with one resting.
