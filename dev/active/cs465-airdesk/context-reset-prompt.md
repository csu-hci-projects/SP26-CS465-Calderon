...
- Recognition V2 pivot: Caden added `deep-research-report.md`; the plan review is complete and the first deterministic per-hand motion-event baseline is implemented at `src/airdesk/gestures/motion.py`. Use `airdesk gesture spot-motion` for replay JSON candidates/diagnostics and `airdesk gesture evaluate-motion` for label evaluation. First bounded replay evidence says this is diagnostic only: `sprint4-swipes-001` default raw-positive-dx mapping matched 0/16 with 12 false activations; flipped mapping matched 5/16 with 12 false activations; stricter flipped dx 1.0 matched 3/16 with 5 false activations; `sprint4-chained-003` default matched 4/10 with 3 repeated fires and 0 false activations. Label-aware motion diagnostics showed background lateral motion can pass motion-only gates, weak-left examples can fall below the displacement gate after tracking dropout/reset, and raw camera sign must stay explicit. Current decision: do not keep polishing the motion baseline forever. The first TCN v2 surface now exists through `build-tcn-dataset --target-mode v2-evidence`, `train-tcn-v2`, `evaluate-tcn-v2`, `diagnose-tcn-v2-events`, and no-action `watch-tcn-v2`. The first old-data v2 smoke validated the surface but not quality: a quick `sprint4-swipes-001` model matched `0/16` at `0.35` thresholds, `1/16` at permissive `0.30` thresholds, and `0/10` on `sprint4-chained-003`. The first staff-level review/refactor chunk is complete: shared hand/no-hand feature stream helpers now drive DTW, motion, TCN dataset windows, and live preview; V2 no-hand/tracking-drop rows stay background-only; V2 start/end targets are scoped to tracked intentional evidence inside each labeled event. Keep live actions disabled.
- First pre-training TCN v2 architecture cleanup, same-source replay check, live feel-test, source-level holdout, and feature-contract audit are complete. New v2 checkpoints are schema `2` residual dilated causal TCNs with per-frame layer normalization/dropout, default 29-frame receptive field, weighted/focal BCE with extra sparse-boundary weighting, per-head calibration threshold metadata, per-head metrics, and batched manifest prediction. Schema-1 v2 checkpoints still load for replay compatibility. `evaluate-tcn-v2` lets `start` boost boundary-backed stroke activation and lets `end` force release/background pressure; it now also supports `--early-match-tolerance-seconds` for causal peaks just before hand-labeled event starts. Same-source old-data replay can score isolated swipes `16/16` with 5 negative false activations, but the new `airdesk gesture holdout-tcn-v2` source split on `sprint4-swipes-001` scored only `2/4` held-out swipes with `5` false activations. New V2 manifests should use `--feature-preset stream-invariant-v2`, which excludes absolute palm position, raw image-space motion, `hand_scale`, `hand_count`, and unscaled finger/pinch geometry from classifier input while preserving those fields in logs/dashboard diagnostics. `airdesk gesture watch-tcn-v2` now gives schema-2 a no-action live/replay dashboard with webcam landmarks, per-hand evidence bars, decoded-gesture history, emit/peak delay, tracker timing, motion-feature diagnostics, start/end-aware decoding, no open-event flushing before release evidence, and JSONL logging through `--events-out`. Next: collect or plan a targeted held-out V2 slice with explicit twist/desk-motion negatives and near/far/left/center/right invariance checks before any live action wiring.
- New public-dataset pivot: the primary-source survey is now in `public-dataset-survey.md`. IPN Hand is the first public-data experiment because it is continuous, RGB-webcam based, CC BY 4.0, includes natural non-gesture hand motion, and maps directly to AirDesk left/right atomic swipe evidence. Jester is larger but remains a later clip-level pretraining candidate. The first importer is `airdesk public-data ipn-convert`; it maps only IPN `G05` / `G06` to AirDesk `swipe_left` / `swipe_right`, writes normal AirDesk replay/labels/features, and can build a `stream-invariant-v2` / `v2-evidence` manifest. The TCN should learn atomic event/boundary evidence (`stroke_left`, `stroke_right`, `start`, `end`, `intentional_motion`), not combo classes such as `right_right_left`. Combos belong in a second command-grammar layer over emitted atomic events. The all-IPN pre-launch review is complete: generated labels match official non-`D0X` events, `D0X` stays background, manifests use `stream-invariant-v2`, CUDA sees the NVIDIA T550, and `scripts/train-ipn-all-tcn-v2.sh` is ready but must not be launched until Caden explicitly confirms. Use `airdesk gesture evaluate-tcn-v2-heads` for the all-IPN held-out metric; `evaluate-tcn-v2` is still the AirDesk swipe decoder.
...
- V2 classifier feature preset: `--feature-preset stream-invariant-v2`;
...
TCN v2, and `watch-tcn-v2` should not stand in as final proof of V2 quality.
...
5. Run `watch-tcn-v2` through the no-action live dashboard and save the JSONL
   output; use evidence bars, decoded gestures, emit/peak delay, timing, and
   motion-feature diagnostics instead of relying on terminal lines alone.
6. Treat wrist-twist/lightbulb motions as first-class targeted negatives. The
   model is position-invariant in the direct-feature sense, but twist can still
   look like swipe motion in projected normalized features.
7. Keep old replay data (`sprint4-swipes-001`, `sprint4-chained-003`, and the
   motion diagnostic files) as a regression suite for sign convention,
   weak-left/tracking-drop, negative-motion false activations, causal early
   peaks, and repeated fires.
8. After that gate, collect a targeted continuous V2 training/test slice:
   repeated same-direction swipes, alternating swipes, weak/tiny lefts, natural
   desk-motion negatives, wrist-twist/lightbulb negatives, hand enters/leaves
   frame, near/far starts, left/right frame-position variants, and two visible
   hands with one resting.
9. Keep broad combo collection paused until that V2 slice has event-level replay
   evidence.
