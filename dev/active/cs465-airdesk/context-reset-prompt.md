...
- New public-dataset planning pivot: do not make AirDesk manually recreate thousands of examples if a public dataset can provide useful atomic motion priors. Next session should survey IPN Hand, Jester, and better candidates if found; IPN Hand is the first likely experiment because it is continuous and includes natural non-gesture motion. The TCN should learn atomic event/boundary evidence (`stroke_left`, `stroke_right`, `start`, `end`, `intentional_motion`), not combo classes such as `right_right_left`. Combos belong in a second command-grammar layer over emitted atomic events.
...
1. Use `holdout-tcn-v2` and `diagnose-tcn-v2-events` before trusting any
   same-source replay score; the first schema-2 source holdout is `2/4` with `5`
   false activations.
2. Run `watch-tcn-v2` through the no-action live dashboard and save the JSONL
...
3. Treat wrist-twist/lightbulb motions as first-class targeted negatives. The
...
4. Keep old replay data (`sprint4-swipes-001`, `sprint4-chained-003`, and the
...
5. After that gate, collect a targeted continuous V2 training/test slice:
...
6. Keep broad combo collection paused until that V2 slice has event-level replay
   evidence.
