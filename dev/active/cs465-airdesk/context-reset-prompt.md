...
- First pre-training TCN v2 architecture cleanup, same-source replay check, live feel-test, and source-level holdout are complete. New v2 checkpoints are schema `2` residual dilated causal TCNs with per-frame layer normalization/dropout, default 29-frame receptive field, weighted/focal BCE with extra sparse-boundary weighting, per-head calibration threshold metadata, per-head metrics, and batched manifest prediction. Schema-1 v2 checkpoints still load for replay compatibility. `evaluate-tcn-v2` lets `start` boost boundary-backed stroke activation and lets `end` force release/background pressure; it now also supports `--early-match-tolerance-seconds` for causal peaks just before hand-labeled event starts. Same-source old-data replay can score isolated swipes `16/16` with 5 negative false activations, but the new `airdesk gesture holdout-tcn-v2` source split on `sprint4-swipes-001` scored only `2/4` held-out swipes with `5` false activations. `airdesk gesture watch-tcn-v2` now gives schema-2 a no-action live/replay dashboard with webcam landmarks, per-hand evidence bars, decoded-gesture history, emit/peak delay, tracker timing, motion-feature diagnostics, start/end-aware decoding, no open-event flushing before release evidence, and JSONL logging through `--events-out`. Live wrist-twist/lightbulb motion can fire as swipes because projected wrist rotation still changes normalized dx, peak velocity, direction consistency, hand scale, and finger-relative motion even without absolute palm position features. Next: collect or plan a targeted held-out V2 slice with explicit twist/desk-motion negatives before any live action wiring.
