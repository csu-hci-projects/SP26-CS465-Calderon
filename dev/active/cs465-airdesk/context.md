...
- May 2026 two-hand pivot: chart combo collection exposed a design flaw. Live collection defaulted to `max_num_hands=1`, and feature export currently uses `frame.hands[0]`, so alternating-hand or both-hands-visible combo recordings can be contaminated. If one hand remains tracked in frame, the other hand's gesture may not become active. The `sprint4-gpu-swipes-002-structured` combo recordings were deleted and should be recollected only after two-hand collection and per-hand feature/evaluation support are implemented. The `sprint4-gpu-swipes-002-singles` batch remains local but should be treated as legacy/single-hand-only until reviewed or recollected with two-hand background/rest conditions.
...
> Stop combo data collection until AirDesk supports two simultaneously visible hands through collection, feature export, DTW/TCN evaluation, and event decoding. Next implementation chunk: record with `--max-num-hands 2`, export per-hand feature rows with independent motion history, score recognizers per hand, merge/decode events across hands, and only then recollect combo/chained swipe charts.
