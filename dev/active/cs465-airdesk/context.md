...
> Pause broad combo collection. Use the 003+004 two-hand batches to tighten weak active-hand labels, timestamp alignment, decoder thresholds, and mirrored-direction conventions. More data is useful only after the next command set is targeted at the observed misses/false activations and still uses `--max-num-hands 2`.
