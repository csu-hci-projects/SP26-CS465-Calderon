...
> Pause broad combo collection. Try the more sensitive stable-HUD `watch-tcn` checkpoint once: `scripts/airdesk-nvidia-mediapipe-wayland gesture watch-tcn --model data/models/gestures/tcn-sprint4-003-004-two-hand-motion-gated020-phase-stroke.pt --max-num-hands 2 --hand-delegate gpu --show --profile-timing --confidence-threshold 0.35`. It is still diagnostic only and may false-fire. If live `dx` is high but `L=`/`R=` remain low, stop pushing the current TCN and pivot to simpler motion/event decoding or a new labeled live calibration slice. More broad combo data is useful only after this targeted failure mode is resolved.
