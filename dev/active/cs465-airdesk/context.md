...
> Combo collection can resume only after full replay validation of the new two-hand path. Record combo charts with `--max-num-hands 2`, export per-hand feature rows with independent motion history, score recognizers per hand, merge/decode events across hands, and then recollect combo/chained swipe charts.
