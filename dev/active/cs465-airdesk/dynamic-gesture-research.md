...
May 2026 Recognition V2 update: after Caden's deep research report and live TCN tests, stop treating the current TCN scaffold as the recognizer architecture. It is still useful evidence, but AirDesk now needs a cleaner continuous-spotting architecture: per-hand normalized streams, motion activity proposal, scorer/model adapter, event decoder, command queue, and mode/profile safety policy. Caden saw live `dx > 0.50` while TCN stroke probabilities stayed flat, so the failure is not only a motion threshold. The plan review is complete, and the first deterministic per-hand motion-event baseline now exists through `airdesk gesture spot-motion` and `airdesk gesture evaluate-motion`. Replay evidence says the baseline is a diagnostic, not a live recognizer: motion-only spotting exposes raw camera direction issues, false activations on negatives, and weak-left/tracking-continuity misses. The first TCN v2 surface now also exists: `build-tcn-dataset --target-mode v2-evidence`, `train-tcn-v2`, `evaluate-tcn-v2`, and `diagnose-tcn-v2-events` define framewise intent/stroke/start/end evidence and replay decoder evaluation. The first pre-training TCN v2 architecture cleanup is also complete: residual dilated causal blocks, per-frame layer normalization/dropout, weighted/focal BCE, sparse-boundary weighting, threshold calibration metadata, schema-versioned checkpoints, batched prediction, and start/end-aware decoder scoring. Schema-2 replay improved isolated old swipes to `16/16` with a 0.25 s causal early-match tolerance, but it still false-activated on 5 negative recordings; chained replay reached `8/10` with 3 repeated fires. `airdesk gesture watch-tcn-v2` now lets schema-2 be tried live as an observation-only dashboard with per-hand evidence bars, recent decoded gestures, emit/peak delay, tracker timing, and JSONL logging, while keeping learned swipes out of desktop actions. Keep the old replay data as a regression suite, use live preview as another diagnostic surface, tighten negative/repeated-fire behavior if it still appears, then collect targeted continuous V2 data rather than treating old isolated/chained data as final proof.
