...
May 2026 Recognition V2 update: after Caden's deep research report and live TCN tests, stop treating the current TCN scaffold as the recognizer architecture. It is still useful evidence, but AirDesk now needs a cleaner continuous-spotting architecture: per-hand normalized streams, motion activity proposal, scorer/model adapter, event decoder, command queue, and mode/profile safety policy. Caden saw live `dx > 0.50` while TCN stroke probabilities stayed flat, so the failure is not only a motion threshold. The plan review is complete, and the first deterministic per-hand motion-event baseline now exists through `airdesk gesture spot-motion` and `airdesk gesture evaluate-motion`. Replay evidence says the baseline is a diagnostic, not a live recognizer: motion-only spotting exposes raw camera direction issues, false activations on negatives, and weak-left/tracking-continuity misses. The first TCN v2 surface now also exists: `build-tcn-dataset --target-mode v2-evidence`, `train-tcn-v2`, `evaluate-tcn-v2`, `diagnose-tcn-v2-events`, and `holdout-tcn-v2` define framewise intent/stroke/start/end evidence, replay decoder evaluation, and source-level train/test scoring. The first pre-training TCN v2 architecture cleanup is also complete: residual dilated causal blocks, per-frame layer normalization/dropout, weighted/focal BCE, sparse-boundary weighting, threshold calibration metadata, schema-versioned checkpoints, batched prediction, and start/end-aware decoder scoring. Schema-2 same-source replay improved isolated old swipes to `16/16` with a 0.25 s causal early-match tolerance, but source-level holdout on the same old batch scored only `2/4` held-out swipes with `5` false activations; chained replay reached `8/10` with 3 repeated fires. The feature-contract audit added `stream-invariant-v2`, which excludes absolute palm position, raw image-space motion, `hand_scale`, `hand_count`, and unscaled finger/pinch geometry from classifier input while keeping those fields in diagnostics. `airdesk gesture watch-tcn-v2` now lets schema-2 be tried live as an observation-only dashboard with per-hand evidence bars, recent decoded gestures, emit/peak delay, tracker timing, motion-feature diagnostics, and JSONL logging, while keeping learned swipes out of desktop actions. Live wrist-twist false positives show why targeted negatives matter even with normalized features. Keep the old replay data as a regression suite, use live preview as another diagnostic surface, then collect targeted held-out V2 data rather than treating old isolated/chained data as final proof.

May 2026 all-IPN live update: the all-gesture IPN model is a useful prior but
not a global AirDesk recognizer. The current best checkpoint,
`tcn-v2-ipn-all-w16-80ep-h64-l4.pt`, scored reasonably on held-out IPN heads,
but live preview showed high-confidence false activations from ordinary
open-hand presence and accidental posture/motion. This reinforces that mode and
profile policy are part of recognition, not a later UI detail. Point/click heads
belong in cursor mode; zoom heads belong in a zoom/media mode; fragile heads
such as `Throw up`, `Open twice`, and `Zoom out` should stay disabled globally
until AirDesk hard-negative data and mode-specific filters support them.
